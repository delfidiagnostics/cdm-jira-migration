...
Works for any user on `POST /issue/{key}/transitions`; returns `403` on `PUT /issue/{key}` for non-admins. Now that admin is granted on both projects, every PUT write appends `NO_NOTIFY` (`?notifyUsers=false`) — labels, resolutions, assignees, parents, convert-subtask re-parent, and the preflight/deprecate epic renames — and the bulk-move sets `sendBulkNotification:false`. This suppresses **only the script's own writes**; the project notification scheme is untouched, so normal ticket creation/edits by users still notify as usual. This replaces the old "mute the CDM notification scheme before running" step (which was fragile anyway — CDM is team-managed and its simplified scheme isn't cleanly swappable via API). Epic *creation* (`POST /issue`) has no `notifyUsers` param, but new epics have no watchers/assignee so it's silent in practice.
...
Adding `Ongoing` to the TESTCDM workflow (UI operation) renumbered the In Progress transition from `21` to `2`, and removed `41` (Backlog) and `2` (Refining) entirely. Transition IDs are workflow-specific and *not stable across UI edits*. On **CDM** the verified live IDs are `11=To Do, 21=In Progress, 31=Done, 51=Dismissed, 52=Ongoing` (Ongoing added to the CDM workflow 2026-05). The only id that differs between projects is **Ongoing** (`3` on TESTCDM, `52` on CDM), so it's pulled from `EPIC_CONFIG[PROJECT]["ongoing_transition"]` and applied to `STATUS_TO_TRANSITION` in `main()`. Zero worksheet rows use Ongoing today, so this is dormant but now correct per project. The cleanest long-term fix is still runtime transition lookup by name (`GET /issue/{key}/transitions`), deferred since no current row triggers it.
